import { ZERO, type PuzzleSymbol, ONE } from "../constants.js";
import type { PuzzleSymbolLineStr } from "../types.js";
import { lineSizeToNumRequired, numRequiredOfValue } from "../utils.js";
import { validateLine } from "../validate/line.js";
import type { LineStrSymbolPermutations } from "./types.js";

function recGenerateAllValidFilledLines(
	length: number,
	...
	if ((lastValue !== ZERO || secondLastValue !== ZERO) && remainingZero > 0) {
		...
		validLines.push(...recGenerateAllValidFilledLines(
			length,
			`${currentLine}${ZERO}`,
			ZERO,
			lastValue,
...
export function generateAllValidFilledLines(length: number, _numRequired?: Record<PuzzleSymbol, number>): LineStrSymbolPermutations {
	const numRequired = _numRequired ?? lineSizeToNumRequired(length);

	return recGenerateAllValidFilledLines(
		length,
		'',
		null,
		null,
...
// Old version, but after benchmarking proved to be about 4x slower than "generateAllValidFilledLines"
