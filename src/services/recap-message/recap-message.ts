...
import { formatLocaleOrdinal } from "@/i18n/format-ordinal.js";
...
// TODO: localize number (percentage and time) formatting, 
export const recapI18nMessageMap: Record<RecapMessageType, (ctx: any, locale: string) => ({ key: string, namedProperties?: Record<string, string | number>}) | null> = { // TODO: any as callback, could be better
    ...
    [RECAP_MSG_TYPES.TIME_RECORD_LARGE]: ({ improvementPercentage }) => ({
        ...
        namedProperties: { improvementPercentage: formatPercentage(improvementPercentage) }
    }),
    ...
    [RECAP_MSG_TYPES.REPLAY_TIME_RECORD]: ({ numPlays, difference }, locale) => ({
        key: "Recap.message.replayTimeRecord",
        namedProperties: { numPlaysOrdinal: formatLocaleOrdinal(numPlays + 1, locale), differenceSec: msToSec(difference) }
    }),
    ...
    [RECAP_MSG_TYPES.ALMOST_TIME_RECORD_PERCENTAGE]: ({ differencePercentage }) => ({
        ...
        namedProperties: { differencePercentage: formatPercentage(differencePercentage) }
    }),
	...
	[RECAP_MSG_TYPES.REPLAY_PLAYS_TOTAL]: ({ bestPreviousTime, numPlays }, locale) => ({
        key: "Recap.message.replayPlaysTotal",
        namedProperties: { bestPreviousTimeMinSec: msToMinSec(bestPreviousTime), numPlaysOrdinal: formatLocaleOrdinal(numPlays + 1, locale) }
    }),
